...
export function prev(node: Exclude<LiquidHtmlNode, DocumentNode>) {
  if (!node.parentNode) return;
  const collection = parentCollection(node);
  return collection[collection.indexOf(node) - 1];
}

export function next(node: Exclude<LiquidHtmlNode, DocumentNode>) {
  if (!node.parentNode) return;
  const collection = parentCollection(node);
  return collection[collection.indexOf(node) + 1];
}
...
function parentCollection(node: Exclude<LiquidHtmlNode, DocumentNode>): LiquidHtmlNode[] {
  if (!node.parentNode) {
    return [];
  }
  ...
  for (const key of COLLECTION_KEYS) {
    ...
    if (key in node.parentNode && Array.isArray((node as any).parentNode[key])) {
